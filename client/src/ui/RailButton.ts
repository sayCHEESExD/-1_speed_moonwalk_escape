...
export class RailButton {
  ...
  constructor(
    parent: HTMLElement,
    options: {
      /** Modifier suffix: `mwe-tile--<variant>` supplies the gradient. */
    ...
    this.root.className = `mwe-tile mwe-tile--${options.variant}`;
    this.root.setAttribute('aria-label', options.label);
    this.root.innerHTML = options.icon;

    const label = document.createElement('span');
    label.className = 'mwe-tile__label mwe-font mwe-outline';
    label.textContent = options.label;
    this.root.appendChild(label);

    if (options.hotkey) {
      const key = document.createElement('span');
      key.className = 'mwe-tile__key mwe-font';
    ...
    this.badge.className = 'mwe-tile__badge mwe-font';
  ...
  setState(ready: boolean, locked = false): void {
    this.root.classList.toggle('mwe-tile--ready', ready);
    this.root.classList.toggle('mwe-tile--locked', locked);
  }
